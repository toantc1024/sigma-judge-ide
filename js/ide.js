...
function showError(title, content) {
    ...
    if(title === "error (429)"){
        $("#site-modal .content").html("<pre>Bạn vui lòng chờ trong giây lát để được xử lý</pre>");
    } else {
...
function handleResult(data) {
    ...
    console.log("Thời gian hoàn tất chạy của IDE: " + (timeEnd - timeStart) + " ms.");
    ...
    var time = (data.time === null ? "-" : data.time + " giây");
    var memory = (data.memory === null ? "-" : data.memory + "KB");
    if(status.description === "Accepted"){
        get_status = "Chấp thuận";
    } 
    else if(status.description === "Compilation Error"){
        get_status = "Lỗi biên dịch";
    } 
    else if(status.description === "Time Limit Exceeded"){
        get_status = "Chạy quá thời gian";
    }
    else{
        get_status = "Lỗi biên dịch";
    }
    $statusLine.html(`${get_status}, ${time}, ${memory}`);
    ...
    stdoutEditor.setValue(stdout);//Giá trị ra
    stderrEditor.setValue(stderr);//Giá trị vào
...
function run() {
    if (sourceEditor.getValue().trim() === "") {
        showError("Báo lỗi", "Code không thể để trống!");
        return;
    } else {
    ...
    var sendRequest = function(data) {
        ...
        $.ajax({
            ...
            success: function (data, textStatus, jqXHR) {
                console.log(`Token là: ${data.token}`);
                if (wait == true) {
                    handleResult(data);
                } else {
...
$(document).ready(function () {
    ...
    $insertTemplateBtn.click(function (e) {
        if (isEditorDirty && confirm("Bạn có muốn khởi động lại? Toàn bộ thay đổi của bạn sẽ bị hủy.")) {
            insertTemplate();
        }
    });
